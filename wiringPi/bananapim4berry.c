...
//
//
//	WiringPi BANANAPI-M4Berry Board Control file (Allwinner 64Bits Platform)
//
//
/*----------------------------------------------------------------------------*/
#include <stdio.h>
...
static int	isBananapiM4BPin (int pin);
...
static int isBananapiM4BPin(int pin)
{
	if (pin >= M4B_GPIO_PIN_BASE && pin <= M4B_GPIO_PIN_END)
		return 1;
	else
		return 0;
}
...
static void sunxi_pwm_set_period(int period_cys)
{
	...
	val = sunxi_pwm_readl(M4B_PWM_PERIOD_REG);
	val &= 0x0000ffff;
	val |= period_cys;
	sunxi_pwm_writel(M4B_PWM_PERIOD_REG, val);
}
...
static void sunxi_pwm_set_mode(int mode)
{
	...
	val = sunxi_pwm_readl(M4B_PWM_CTRL_REG);
	if(mode)
	{
		//pulse mode, PWM_MODE_BAL
		val |= (M4B_PWM_MODE | M4B_PWM_PUL_START);
		m4b_pwmmode = 1;
	}
	else
	{
	...
	sunxi_pwm_writel(M4B_PWM_CTRL_REG, val);
}
...
static void sunxi_pwm_set_enable(int enable)
{
	...
	//sclk gating
	val = sunxi_pwm_readl(M4B_PWM_CLK_REG);
	if(enable)
		val |= M4B_PWM_SCLK_GATING;
	else
		val &= ~(M4B_PWM_SCLK_GATING);
	sunxi_pwm_writel(M4B_PWM_CLK_REG, val);

	//pwm enable
	val = sunxi_pwm_readl(M4B_PWM_EN_REG);
	if(enable)
		val |= M4B_PWM_EN;
	else
		val &= ~(M4B_PWM_EN);
	sunxi_pwm_writel(M4B_PWM_EN_REG, val);
}

static void sunxi_pwm_set_all()
{
	//clear all reg
	sunxi_pwm_writel(M4B_PWM_CTRL_REG, 0);
	sunxi_pwm_writel(M4B_PWM_PERIOD_REG, 0);
...
static int _getModeToGpio (int mode, int pin)
{
	...
	/* Native gpio number */
	case MODE_GPIO:
		retPin = isBananapiM4BPin(pin) ? pin : -1;
		break;
...
static int _pwmToneWrite(int pin, int freq)
{
	...
	if (freq == 0) {
		...
		div = sunxi_pwm_readl(M4B_PWM_CTRL_REG);
...
static void init_gpio_mmap (void)
{
	...
	if (fd < 0) {
		...
		mapped_gpio = mmap(0, BLOCK_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, fd, M4B_GPIO_BASE);
		if (mapped_gpio == MAP_FAILED)
			msg(MSG_ERR, "wiringPiSetup: mmap (GPIO) failed: %s \n", strerror (errno));
		else
			gpio = (uint32_t *) mapped_gpio;

		mapped_pwm = mmap(0, BLOCK_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, fd, M4B_PWM_BASE);
		if (mapped_gpio == MAP_FAILED)
